...
class RetryEligibilityTests(unittest.TestCase):
    ...
    def test_summarize_bugbot_gate_is_optional_by_default(self):
    ...
    def test_summarize_bugbot_gate_can_be_required(self):
        gate = watch.summarize_bugbot_gate_from_checks([], required=True)
    ...
    def test_fetch_new_review_items_does_not_block_all_comments_when_lookup_truncated(self):
        pr = {
            "repo": "rock3r/fineco-helper",
            "number": 716,
            "head_sha": "abc123",
        ...
        with patch.object(
            ...
            watch,
            "get_unresolved_review_comment_ids",
            return_value={"ids": set(), "truncated": True},
        ):
            new_items, blocking_items = watch.fetch_new_review_items(
                pr,
                state,
    ...
    def _own_review_comment_payload(self):
        return [
            {
                "id": 7,
                "user": {"login": "octocat"},
    ...
    def test_fetch_new_review_items_blocks_on_own_unresolved_threads(self):
        ...
        pr = {"repo": "rock3r/fineco-helper", "number": 716, "head_sha": "abc123"}
        state = {
            "seen_issue_comment_ids": [],
            "seen_review_comment_ids": [],
            "seen_review_ids": [],
        ...
        with patch.object(
            watch,
            "gh_api_list_paginated",
            side_effect=[[], self._own_review_comment_payload(), []],
        ), patch.object(
            watch,
            "get_unresolved_review_comment_ids",
    ...
    def test_fetch_new_review_items_ignores_codex_review_summary_status_comment(self):
        ...
        # finding, so it must not resurface as a new or blocking item.
        pr = {"repo": "rock3r/fineco-helper", "number": 716, "head_sha": "abc123"}
        state = {
            "seen_issue_comment_ids": [],
            "seen_review_comment_ids": [],
        ...
        issue_payload = [
            ...
            },
            {
                ...
                "created_at": "2026-01-01T00:00:00Z",
                "updated_at": "2026-01-01T00:00:00Z",
                "body": "**P2** Close the socket on the error path.",
        ...
        self.assertEqual([item["id"] for item in blocking_items], ["4"])

    def test_fetch_new_review_items_keeps_seen_issue_comment_blocking(self):
        pr = {
            "repo": "rock3r/fineco-helper",
            "number": 716,
            "head_sha": "abc123",
        ...
        self.assertEqual(new_items, [])
        self.assertEqual(len(blocking_items), 1)
        self.assertEqual(blocking_items[0]["id"], "5")
    ...
    def test_summarize_codex_gate_uses_paginated_reactions(self):
        reactions = [
            {"content": "+1", "user": {"login": "maintainer"}},
            {"content": "eyes", "user": {"login": "chatgpt-codex-connector[bot]"}},
        ]

        with patch.object(watch, "gh_api_list_paginated", return_value=reactions) as paginated:
            gate = watch.summarize_codex_gate("rock3r/fineco-helper", 716)

        paginated.assert_called_once()
        self.assertTrue(gate["reviewing"])
        self.assertEqual(gate["status"], "in_progress")

    def test_summarize_codex_gate_fails_closed_on_api_error(self):
        with patch.object(
            watch,
            "gh_api_list_paginated",
    ...
    def test_reset_state_for_new_head_sha_clears_pending_map(self):
        ...
        self.assertEqual(state["blocking_issue_comment_ids"], [])
    ...
    def test_load_state_resets_seen_tracking_when_stale(self):
        ...
        self.assertEqual(loaded["blocking_issue_comment_ids"], [])
    ...
    def test_should_stop_watching_waits_when_conflict_and_codex_running(self):
    ...
    def test_run_watch_timeout_includes_final_snapshot(self):
        events = []
        actionable = {
            "pr": {"closed": False, "merged": False},
            "checks": {"all_terminal": True, "failed_count": 0, "pending_count": 0, "passed_count": 2},
            "actions": ["diagnose_branch_behind"],
        }

        with patch.object(watch.time, "time", side_effect=[0, 61]), \
             patch.object(watch, "collect_snapshot", return_value=(actionable, watch.Path("/tmp/s.json"))), \
             patch.object(watch, "print_event", side_effect=lambda event, payload: events.append((event, payload))):
        ...
        self.assertIn("diagnose_branch_behind", payload["actions"])
        self.assertIn("stop_session_timeout", payload["actions"])
...
class NeedsAgentAttentionTests(unittest.TestCase):
    ...
    def test_conflict_waiting_on_bot_does_not_need_attention(self):
        self.assertFalse(watch.needs_agent_attention(["diagnose_merge_conflict", "wait_codex"]))
        self.assertFalse(watch.needs_agent_attention(["diagnose_merge_conflict", "wait_bugbot"]))

    def test_branch_behind_waiting_on_bot_does_not_need_attention(self):
        self.assertFalse(watch.needs_agent_attention(["diagnose_branch_behind", "wait_codex"]))
        self.assertFalse(watch.needs_agent_attention(["diagnose_branch_behind", "wait_bugbot"]))

    def test_bot_wait_does_not_hide_other_actionable_states(self):
        self.assertTrue(
            watch.needs_agent_attention(
                ["diagnose_branch_behind", "wait_codex", "process_review_comment"]
            )
        )
        self.assertTrue(
            watch.needs_agent_attention(
                ["diagnose_merge_conflict", "wait_bugbot", "diagnose_ci_failure"]
            )
        )
...
class GhTextTests(unittest.TestCase):
    def test_gh_text_times_out(self):
        with patch.object(
            watch.subprocess,
            "run",
            side_effect=subprocess.TimeoutExpired(["gh", "pr", "view"], watch.GH_COMMAND_TIMEOUT_SECONDS),
        ):
    ...
    def test_get_pr_checks_reads_json_when_checks_are_pending_or_failing(self):
        ...
        for code in (1, 8):
            with patch.object(watch.subprocess, "run", side_effect=self._fake_gh_run(code, payload)):
                checks = watch.get_pr_checks("21", repo="rock3r/fineco-helper")
    ...
    def test_get_pr_checks_still_fails_without_json(self):
        ...
        with patch.object(watch.subprocess, "run", side_effect=fake):
            with self.assertRaises(watch.GhCommandError):
                watch.get_pr_checks("21", repo="rock3r/fineco-helper")

    def test_gh_text_still_fails_on_other_exit_codes_with_output(self):
        fake = self._fake_gh_run(4, '{"message": "auth required"}')
        with patch.object(watch.subprocess, "run", side_effect=fake):
            with self.assertRaises(watch.GhCommandError):
                watch.get_pr_checks("21", repo="rock3r/fineco-helper")
...
class RunOnceTests(unittest.TestCase):
    ...
    def test_session_timeout_preserves_snapshot_actions(self):
        actionable = self._actionable_snapshot()

        with patch.object(watch, "collect_snapshot", return_value=(actionable, watch.Path("/tmp/s.json"))), \
             patch.object(watch.time, "time", side_effect=[0, 1]):
            result = watch.run_once(SimpleNamespace(
                pr="auto", repo=None, state_file=None, poll_seconds=30, max_flaky_retries=3, max_session_minutes=0,
            ))

        self.assertIn("diagnose_ci_failure", result["actions"])
        self.assertIn("stop_non_retryable_failure", result["actions"])
        self.assertIn("stop_session_timeout", result["actions"])
...
class SkippingChecksTests(unittest.TestCase):
    ...
    def test_recommend_actions_surfaces_branch_behind(self):
        pr = {
            "closed": False,
            "merged": False,
            "mergeable": "MERGEABLE",
    ...
    def test_should_stop_watching_on_branch_behind(self):
        self.assertTrue(watch.should_stop_watching(["diagnose_branch_behind"]))

    def test_should_not_stop_watching_on_branch_behind_while_bot_running(self):
        self.assertFalse(watch.should_stop_watching(["diagnose_branch_behind", "wait_codex"]))
        self.assertFalse(watch.should_stop_watching(["diagnose_branch_behind", "wait_bugbot"]))

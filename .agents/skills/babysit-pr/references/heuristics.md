...
- Static analysis or typecheck violations introduced by the latest push
...
4. **Push once** only when: all fixes are done AND all review bots are done AND local validation is green. Use the commands appropriate for this repo's current toolchain, such as `go test ./...`, `npm test`, or `npm run lint` once those tools exist.
...
> **Cost rule**: every push can trigger CI and bot review runs. Batch all local fixes — CI failures,
> bot comments, and human review comments — into a single commit before pushing.
...
- After fixing, run local validation to confirm no regressions.
...
- Local validation fails after an attempted fix and the cause is not obvious.

...
`gh pr checks` can exit 1 (a check failed) or 8 (checks pending) and still print the requested JSON.
The watcher reads that JSON instead of treating those exit codes as command failures.
